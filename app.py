...
if uploaded_file:
    ...
    # === DOWNLOAD SUMMARY ANALISIS TXT ===
    st.markdown("---")
    st.markdown("### 📥 Unduh Ringkasan Analisis (TXT)")

    summary_clean_text = summary_text.replace("###", "").replace("*", "").replace("**", "").strip()
    summary_bytes = summary_clean_text.encode("utf-8")

    st.download_button(
        label="⬇️ Unduh Ringkasan Analisis",
        data=summary_bytes,
        file_name="Ringkasan_Analisis.txt",
        mime="text/plain"
    )
else:
